...
static simd_float1 (^gaussian_distribution)(simd_float1, simd_float1, simd_float1, simd_float1, simd_float1) = ^ simd_float1 (simd_float1 x, simd_float1 mean, simd_float1 variance, simd_float1 std_dev, simd_float1 duration) {
//    simd_float1 j = (exp(-((pow((x - mean), 2.f) / pow(2.f * variance, 2.f))))); //(1.f / (std_dev * sqrt(2.f * M_PI))) * (exp(-((pow((x - mean), 2.f) / pow(2.f * variance, 2.f))))); // divide a mean of .5 or less by 4 to get a variance value that extends to the x = 0
...
static void (^(^signal_sample_generator)(float * _Nonnull const * _Nonnull, AVAudioFrameCount, simd_float1 *))(void) = ^ (float * _Nonnull const * _Nonnull float_channel_data, AVAudioFrameCount buffer_length, simd_float1 * normalized_time) {
   AVAudioFrameCount frame = 0;
   AVAudioFrameCount * frame_t = &frame;
   __block simd_float1 time;
   __block simd_float2 tone_durations;
   ...
   return ^{
      ...
      for (*frame_t = 0; *frame_t < buffer_length; *frame_t += 1) {
         ({
            time = *((simd_float1 *)normalized_time + (*frame_t));
            tones = simd_make_double2(simd_make_double2(0.5f * _simd_sin_d2(simd_make_double2( ({ (frequency_theta_v = simd_make_double2(frequency_theta_v + frequency_theta_increment_v)); }) ))) *
            simd_make_double2(0.5f * _simd_sin_d2(simd_make_double2( ({ (envelope_theta_v  = simd_make_double2(envelope_theta_v + envelope_theta_increment_v));    }) ))));
            ({
...
static typeof(audio_buffer_ref) (^audio_buffer)(AVAudioFormat *) = ^ (AVAudioFormat * buffer_format) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        simd_float1 normalized_time[frame_count];
        simd_float1 * normalized_time_t = &normalized_time[0];
        for (AVAudioFrameCount frame = 0; frame < frame_count; frame++) {
            *((simd_float1 *)normalized_time_t + frame) = simd_smoothstep((simd_float1)0.f, (simd_float1)frame_count, (simd_float1)frame);
        }
